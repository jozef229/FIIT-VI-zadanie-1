...
class CsfdSpider(scrapy.Spider):
    ...
    def parse_film(self, response):
        ...
        try:
            film_length = response.css('p.origin::text').extract()[
                1].replace(', ', '').replace(' min', '')
        except:
            print("An exception occurred")

        array_content = response.xpath(
            '//*[@id="plots"]/div[2]/ul/li[1]/div[1]/text()[2]').getall()
        for i, element in enumerate(array_content):
            array_content[i] = element.replace('\t', '').replace('\n', '')

        for i in range(10):
            x1_name = response.xpath(
                '//*[@class="creators"]/div[' + str(i+1) + ']/h4/text()').get()
            if x1_name == 'Režie:':
                administration = response.xpath(
                    '//*[@class="creators"]/div[' + str(i+1) + ']/span[1]/a/text()').getall()
            if x1_name == 'Scénář:':
                scenario = response.xpath(
                    '//*[@class="creators"]/div[' + str(i+1) + ']/span[1]/a/text()').getall()
            if x1_name == 'Kamera:':
                camera = response.xpath(
                    '//*[@class="creators"]/div[' + str(i+1) + ']/span[1]/a/text()').getall()
            if x1_name == 'Hudba:':
        ...
        for i, element in enumerate(release_date):
            help_element = element.replace('\t', '').replace(
                '\n', '').replace(' ', '#', 1).split('#')[0]
            help_element = help_element.split(
                '.')[2] + '-' + help_element.split('.')[1] + '-' + help_element.split('.')[0]
